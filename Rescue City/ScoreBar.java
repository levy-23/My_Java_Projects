...
public class ScoreBar extends Actor
{
    ...
    public ScoreBar (int width)
    {
        ...
        textFont = new Font ("Courier New", 16);
     ...
     */
    public void update (int robbers, int arsonists, int arrests, int fires, int robberies)
    {
        // In order to make uniform sizes and preceding zeros:
        String robbersString, arsonistsString, arrestsString, firesString, robberiesString, crimesString;
        // If there is only one digit
        robbersString = zeroAdder(robbers, 3);
        arsonistsString = zeroAdder (arsonists, 3);
        arrestsString = zeroAdder (arrests, 3);
        firesString = zeroAdder (fires, 3);
        robberiesString = zeroAdder (robberies, 3);
        
        text = "                                    ROBBERS: " + robbersString + "  ARSONISTS: " + arsonistsString + " ARRESTS: " + arrestsString + " FIRES: " + firesString + " ROBBERIES: " + robberiesString;
